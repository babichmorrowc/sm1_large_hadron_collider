...
Additionally this project comes with a package which contains all external annotated `R` code which can be downloaded via the following command in the `R` console:
```{r,eval = FALSE}
...
We see in the above data that removing algebraic codependencies and the uniform variables produced the highest AMS when employing an RBF kernel. With using the top ten highest mutual information actually underperforming against using all variables. This is dissapointing, however not particularly surprising as this cut 2/3 of the variables out of the problem.
