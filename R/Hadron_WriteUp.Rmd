...
The following is the list of the variables with the lowest mutual information which we can remove to reduce our dimensions.

```{r}
#source(here("R/Mutual_Information"))
```

# Fisher Discriminant Analysis

FDA tells us how separated our data can possibly be. This can be a useful aid to find out whether a linear classifier is appropriate and can additionally be used to see how removing variables affects the separability of our data.

We consider three increasing cases of variables elimination: 1) Removing all 4 algebraic co-dependencies 2) Removing the above and any uniform variables. 3) Removing the above and the 10 variables with lowest mutual information.

```{r,echo = FALSE, warning=FALSE,message = FALSE,fig.width = 14,fig.height= 5}
source(here("R/fda.R"))
FDA_all <- ggplot() +
  geom_histogram(aes(x = LD1, y = stat(density), fill = Label),
                 data = higgs_vars_all_pred_scaled)+ggtitle("FDA with all variables")
FDA_unif <- ggplot() +
  geom_histogram(aes(x = LD1, y = stat(density), fill = Label), data = higgs_vars_drop_unif_pred_scaled)+ggtitle("FDA without uniform variables")
FDA_lin <- ggplot() +
  geom_histogram(aes(x = LD1, y = stat(density), fill = Label), data = higgs_vars_drop_combo_pred_scaled)+ggtitle("FDA without colinear and uniform variables")

plot_grid(FDA_all,FDA_unif,FDA_lin,nrow = 1)
```
