...
## Co-Linearity of Data

Multiple of our variables are dependent on one or another, possibly to assist in computation at CERN. However since we are classifying we want to remove as many dimensions as possible. We have the following co-linearities:

-   `PRI_jet_all_pt = PRI_leading_jet_phi + PRI_subleading_pt`

-   Understanding these co-linearities allows us to reject some subset of them to lower the dimension of our data, however as a choice needs to be made as to which to remove. For example we could only choose to keep `PRI_jet_all_pt` and hope that the sum contains enough information from the other two, or we could choose to only remove `PRI_jet_all_pt` and guarantee no information loss, but at the cost of one more dimension.
...
-   We can immediately see that `PRI_tau_phi`,`PRI_lep_phi`, `PRI_met_phi` and `PRI_jet_leading_phi` are uniformly distributed and that there is very little variation between the classifications. This leads us to consider rejecting them from our classification, reducing the dimension by 4.
...
-   We can see extreme variation in the peaks in multiple other variables, indicating that that may be principle components for our analysis.
