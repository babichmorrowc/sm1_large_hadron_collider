...
```{r,echo = FALSE, warning=FALSE,message = FALSE,fig.height= 8,fig.width= 10,fig.align = "center",results=FALSE}
...
From these we can see that using the missing data gives us a slightly improved separability.

If we look exclusively at the contribution of each variable to each principle component when we include the missing data, then we can see that within the first four principle components we have a greater variance in these more uniform variables. Since this is based on correlation of variables across the full dataset it is difficult to determine much further about the separability of the dataset. However, we can see that the first three PCs do not have much contribution from the variables with the lowest mutual information showing that they are unlikely to be useful to include.
...
2) Remove all 4 algebraic co-dependencies (`PRI_jet_leading_pt`, `PRI_jet_subleading_pt`, `DER_pt_ratio_lep_tau`, `PRI_tau_pt`).

3) Remove the above and any uniform variables (`PRI_lep_phi`, `PRI_met_phi`, `PRI_tau_phi`).
