...
```{r}
higgs_data_orig <- read_csv(here("data/atlas-higgs-challenge-2014-v2.csv"),show_col_types = F)
...
-   `PRI_jet_all_pt = PRI_leading_jet_phi + PRI_subleading_pt`
-   `DER_pt_ratio_lep_tau = PRI_lep_pt/PRI_tau_pt`
-   Jet related variables depend on `PRI_jet_num` (see next section for more)

This allows us to reduce the dimension of our data by removing some redundant subset of our variables. However there will be a trade off between dimension reduction and detail. If we are liberal we can remove 2+2+10 = 14 dimensions, however this would probably limit the classifier.
...
To get an understanding of each variable given their classification as either $b$ or $s$ we plot their densities below.

```{r setup, warning=FALSE,,message = FALSE, echo = FALSE,fig.align="center", echo = FALSE,fig.width = 14}
...
ggarrange(plotlist = plot_list,ncol = 2, nrow = 2, align = "v", combine.legend = TRUE)
```

Some observations:

-   We see that `PRI_tau_phi`,`PRI_lep_phi`, `PRI_met_phi` and `PRI_jet_leading_phi` are uniformly distributed and that there is very little variation between the classifications. This leads us to consider rejecting them from our classification, reducing the dimension by 4.

-   We can see the co-linearity of `PRI_jet_all_pt`,`PRI_leading_jet_phi` and `PRI_subleading_pt`, as previously stated we will reject some subset of these for our classification.

# Fisher Discriminant Analysis

In order to get an idea how separated our data is we perform FDA, however considering our previous study of the distribution of each variable we will perform it under different conditions
...
FDA using all variables and including missing data as `-999`: Note that we get an error about collinear variables in our dataset.

```{r}
ggplot() +
  geom_histogram(aes(x = LD1, y = stat(density), fill = Label),
                 data = higgs_vars_all_pred_scaled)
```

## Rejecting angle variables

Notice that in both cases our data is not separated well at all, and there is not much difference between including or excluding missing data points.

We still get the error about collinearity.

```{r}
ggplot() +
  geom_histogram(aes(x = LD1, y = stat(density), fill = Label), data = higgs_vars_drop_unif_pred_scaled)
```


### Keeping Co-linearities

This is where we keep colinearites

### Rejecting Co-linearities

Removing `PRI_jet_leading_pt`, `PRI_jet_subleading_pt`, `PRI_lep_pt`, and `PRI_tau_pt`, as well as the uniform angle variables. We no longer get the error about collinearity.

```{r}
ggplot() +
  geom_histogram(aes(x = LD1, y = stat(density), fill = Label), data = higgs_vars_drop_combo_pred_scaled)
```
